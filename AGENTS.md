...
The VAD model (`silero_vad_v4.onnx`) is already bundled at `src-tauri/resources/models/`.

The ASR model (Qwen3-ASR-0.6B) is handled by the Python sidecar — it loads from ModelScope cache at `~/.cache/modelscope/hub/models/Qwen/Qwen3-ASR-0___6B/`.

```bash
# Set up Python sidecar (one-time)
bash python_sidecar/setup.sh
# Download Qwen3-ASR from ModelScope
...
- `python_sidecar.rs` - Python sidecar subprocess manager (spawn, IPC, restart)
...
**Pipeline Processing:** Audio → VAD → Qwen3-ASR (Python sidecar) → Text output → Clipboard/Paste
...
- `sherpa-onnx` (removed) → **Python sidecar**: Qwen3-ASR via PyTorch CUDA

### Python Sidecar

The ASR inference runs in a separate Python subprocess (`python_sidecar/`):

- `python_sidecar/transcriber.py` — Reads audio (base64 PCM) via stdin JSON, returns text via stdout JSON
- `python_sidecar/venv/` — Python 3.12 + PyTorch CUDA + qwen-asr
- `src-tauri/src/python_sidecar.rs` — Rust subprocess manager (spawn, IPC, restart)
- Model sourced from **ModelScope** (`Qwen/Qwen3-ASR-0.6B`)
