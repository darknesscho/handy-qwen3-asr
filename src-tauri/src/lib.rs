...
fn initialize_core_logic(app_handle: &AppHandle) {
    ...
    // Detect XKB key remapping (e.g. Ctrl↔Win swap) so ydotool sends the
    // correct raw keycode that resolves to the Control modifier at paste time.
    #[cfg(target_os = "linux")]
    crate::utils::detect_and_cache_ctrl_keycode();
...
pub fn run(cli_args: CliArgs) {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            // Pre-warm accelerator enumeration on a background thread.
            std::thread::spawn(|| {
            ...
            // Handle CLI flags for first-instance scenario.
            // This covers: (1) app not running yet, (2) D-Bus IPC failed so
            // second instance degraded to first-instance startup.
            if cli_args.toggle_transcription {
                signal_handle::send_transcription_input(&app_handle, "transcribe", "CLI");
