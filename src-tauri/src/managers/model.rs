...
impl ModelManager {
    pub fn new(app_handle: &AppHandle) -> Result<Self> {
        ...
        // Qwen3-ASR supported languages (52 languages/dialects from sherpa-onnx)
        let qwen3_languages: Vec<String> = vec![
            "en", "zh", "zh-Hans", "zh-Hant", "ja", "ko", "de", "fr", "es", "pt", "it", "ru",
            "ar", "hi", "vi", "th", "tr", "nl", "pl", "sv", "da", "fi", "no", "cs", "hu",
            "ro", "el", "he", "id", "ms", "uk", "bg", "hr", "lt", "lv", "et", "sk", "sl",
            "sr", "ca", "eu", "gl", "sw", "af", "bn", "ta", "te", "mr", "gu", "ml", "kn",
            "si",
        ]
        .into_iter()
        .map(String::from)
    ...
    fn update_download_status(&self) -> Result<()> {
        ...
        for model in models.values_mut() {
            if model.is_directory {
                ...
                // Get partial file size if it exists (for the archive being downloaded)
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<()> {
        ...
        if model_info.is_directory {
            ...
            // Open and extract the downloaded archive (.tar.gz)
