    ...
    pub fn stop_recording(&self, binding_id: &str) -> Option<Vec<f32>> {
        ...
        match *state {
            RecordingState::Recording {
                ...
                // Now that rec.stop() is complete, release the Recording state
                // so try_start_recording() can proceed. Keeping state held until
                // here prevents a race where a new recording starts before the
                // old one finishes.
