...
fn show_overlay_state(app_handle: &AppHandle, state: &str) {
    ...
    // Only update position when the overlay is first shown, not on subsequent
    // state transitions (recording → transcribing → processing). In GNOME
    // Wayland, update_overlay_position triggers GTK Layer Shell anchor
    // reconfiguration which can cause Mutter to re-evaluate window focus,
    // interfering with the subsequent paste operation.
