...
#[cfg(target_os = "linux")]
use crate::utils::{get_cached_ctrl_keycode, is_gnome, is_kde_wayland, is_wayland};
...
fn try_send_key_combo_linux(paste_method: &PasteMethod) -> Result<bool, String> {
    if is_wayland() {
        // Wayland: prefer wtype (but not on KDE or GNOME, where the
        // wlr-virtual-keyboard protocol is unsupported), then dotool, then ydotool
        if !is_kde_wayland() && !is_gnome() && is_wtype_available() {
            match send_key_combo_via_wtype(paste_method) {
                Ok(()) => return Ok(true),
                Err(e) => info!("wtype failed: {e}, trying next tool"),
            }
        }
        if is_dotool_available() {
            match send_key_combo_via_dotool(paste_method) {
                Ok(()) => return Ok(true),
                Err(e) => info!("dotool failed: {e}, trying next tool"),
            }
        }
        if is_ydotool_available() {
            match send_key_combo_via_ydotool(paste_method) {
                Ok(()) => return Ok(true),
                Err(e) => info!("ydotool failed: {e}, trying next tool"),
            }
        }
    } else {
        ...
        if is_xdotool_available() {
            match send_key_combo_via_xdotool(paste_method) {
                Ok(()) => return Ok(true),
                Err(e) => info!("xdotool failed: {e}, trying next tool"),
            }
        }
        if is_ydotool_available() {
            match send_key_combo_via_ydotool(paste_method) {
...
fn send_key_combo_via_ydotool(paste_method: &PasteMethod) -> Result<(), String> {
    ...
    // where pressed is 1 for down, 0 for up.
    //   KEY_LEFTCTRL = 29  (adjustable via get_cached_ctrl_keycode for XKB swaps)
    //   KEY_LEFTSHIFT = 42
    //   KEY_V = 47
    //   KEY_INSERT = 110
    let ctrl = get_cached_ctrl_keycode();
    let ctrl_down = format!("{ctrl}:1");
    let ctrl_up = format!("{ctrl}:0");

    let args: Vec<String> = match paste_method {
        ...
        PasteMethod::ShiftInsert => {
            vec!["key".into(), "42:1".into(), "110:1".into(), "110:0".into(), "42:0".into()]
        }
