...
impl AudioRecorder {
        ...
        f32: cpal::FromSample<T>,
    {
        ...
        let stream_cb = move |data: &[T], _: &cpal::InputCallbackInfo| {
            // Extract audio before checking stop_flag so the final buffer is
            // captured alongside the EndOfStream sentinel, not discarded.
            output_buffer.clear();
...
fn run_consumer(
    ...
    loop {
        // Always check for pending commands first so they are not dropped
        // when recv_timeout fires before the first audio chunk arrives.
        ...
        let chunk = match sample_rx.recv_timeout(Duration::from_secs(1)) {
            Ok(c) => c,
